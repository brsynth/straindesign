...
from cameobrs import config, fba
from cameobrs.core.manipulation import swap_cofactors
from cameobrs.parallel import SequentialView
from cameobrs.strain_design import OptGene
from cameobrs.strain_design.heuristic.evolutionary.archives import (
    BestSolutionArchive,
    Individual,
)
from cameobrs.strain_design.heuristic.evolutionary.decoders import (
    GeneSetDecoder,
    ReactionSetDecoder,
    SetDecoder,
)
from cameobrs.strain_design.heuristic.evolutionary.evaluators import KnockoutEvaluator
from cameobrs.strain_design.heuristic.evolutionary.generators import (
    linear_set_generator,
    multiple_chromosome_set_generator,
    set_generator,
)
from cameobrs.strain_design.heuristic.evolutionary.genomes import (
    MultipleChromosomeGenome,
)
from cameobrs.strain_design.heuristic.evolutionary.metrics import (
    euclidean_distance,
    manhattan_distance,
)
from cameobrs.strain_design.heuristic.evolutionary.multiprocess.migrators import (
    MultiprocessingMigrator,
)
from cameobrs.strain_design.heuristic.evolutionary.objective_functions import (
...
from cameobrs.strain_design.heuristic.evolutionary.variators import (
...
from cameobrs.util import RandomGenerator as Random
...
def objectives():
    ...
    multi_objective_function = MultiObjectiveFunction(
        [product_yield("product", "substrate"), number_of_knockouts()]
    )
...
class TestBestSolutionArchive:
    ...
    def test_solution_comparison_maximization(self):
        ...
        # test difference
        li = len(sol2.symmetric_difference(sol1))
        assert (
            li == 1
        ), "Difference between Solution 2 and 1 is (%s)" % sol2.symmetric_difference(
            sol1
        )
        li = len(sol3.symmetric_difference(sol2))
        assert (
            li == 1
        ), "Difference between Solution 3 and 1 is (%s)" % sol3.symmetric_difference(
            sol2
        )
        li = len(sol3.symmetric_difference(sol1))
        assert (
            li == 2
        ), "Difference between Solution 1 and 3 is (%s)" % sol3.symmetric_difference(
            sol1
        )
    ...
    def test_add_greater_solution_with_same_fitness(self):
        ...
        assert pool.length() == 1, (
            "Pool must keep one solution (length=%s)" % pool.length()
        )
    ...
    def test_add_smaller_solution_with_same_fitness(self):
        ...
        sol = pool.get(0)
        assert sol.candidate == solution, (
            "Best solution must be the first (%s)" % sol.candidate
        )
        assert sol.fitness == fitness, (
            "Best fitness must be the first (%s)" % sol.fitness
        )
    ...
    def test_callable_pool(self):
        ...
        population = [
            Individual(SOLUTIONS[0][0], SOLUTIONS[0][1]),
...
class TestObjectiveFunctions:
    ...
    def test_base_yield_function(self, model):
        ...
        of = YieldFunction(
            model.reactions.EX_ac_lp_e_rp_, model.reactions.EX_glc_lp_e_rp_
        )
    ...
    def test_biomass_product_coupled_yield(self):
        ...
        of2 = biomass_product_coupled_yield(
            "biomass", "product", ["substrate", "substrate2"]
        )
    ...
    def test_biomass_product_coupled_min_yield(self, model):
        ...
        reactions = [
            ...
            for r in ["ATPS4r", "CO2t", "GLUDy", "PPS", "PYK"]
        ]
        with model:
            for r in reactions:
                r.knock_out()
...
class TestKnockoutEvaluator:
    def test_initializer(self, model):
        ...
        decoder = ReactionSetDecoder(
            ["PGI", "PDH", "FUM", "FBA", "G6PDH2r", "FRD7", "PGL", "PPC"], model
        )
        ...
        evaluator = KnockoutEvaluator(
            model, decoder, MultiObjectiveFunction([objective1, objective2]), fba, {}
        )
    ...
    def test_evaluate_multi_objective(self, model):
        ...
        objective2 = product_yield(
            "EX_ac_lp_e_rp_", "EX_glc_lp_e_rp_", carbon_yield=False
        )
...
class TestSwapOptimization:
    def test_swap_reaction_identification(self, model):
        ...
        swap_pairs = (
            [model.metabolites.get_by_id(m) for m in NADH_NADPH[0]],
            [model.metabolites.get_by_id(m) for m in NADH_NADPH[1]],
        )

        representation = CofactorSwapOptimization.find_swappable_reactions(
            model, swap_pairs
        )
    ...
    def test_evaluate_swap(self, model):
        cofactors = (
            (model.metabolites.nad_c, model.metabolites.nadh_c),
        ...
        with model:
            model.reactions.Biomass_Ecoli_core_N_lp_w_fsh_GAM_rp__Nmet2.lower_bound = (
                0.5
            )
            py = product_yield(
                model.reactions.EX_etoh_lp_e_rp_, model.reactions.EX_glc_lp_e_rp_
            )
            model.objective = model.reactions.EX_etoh_lp_e_rp_
            swap_cofactors(model.reactions.ALCD2x, model, cofactors, inplace=True)
            reactions = ["GAPD", "AKGDH", "PDH", "GLUDy", "MDH"]
            optimization = CofactorSwapOptimization(
                model=model, objective_function=py, candidate_reactions=reactions
            )
...
class TestDecoders:
    ...
    def test_reaction_set_decoder_with_groups(self, model):
        groups = [
            {model.reactions[1]: 1, model.reactions[11]: 1, model.reactions[12]: 5},
            {model.reactions[2]: 1, model.reactions[13]: 1, model.reactions[14]: 5},
        ]

        decoder = ReactionSetDecoder(
            [r.id for r in model.reactions[0:10]], model, groups=groups
        )
...
class TestGenerators:
    ...
    def test_fixed_size_set_generator(self, generators, fix_size_candidates):
        args, random, _ = generators
        candidates_file = fix_size_candidates
        random.seed(SEED)
    ...
    def test_variable_size_set_generator(self, generators, variable_size_candidates):
        args, random, _ = generators
        candidates_file = variable_size_candidates
...
class TestHeuristicOptimization:
    def test_default_initializer(self, model, objectives):
        ...
        heuristic_optimization = HeuristicOptimization(
            model=model, objective_function=single_objective_function
        )
    ...
    def test_single_objective_function_with_multiobjective_initializer(
...
class TestOptimizationResult:
    def test_reaction_result(self, model):
        ...
        for row in result:
            ...
            assert solutions.archive.count(individual) == 1, (
                "%s is unique in archive" % individual
            )
...
def reaction_ko_single_objective(model):
    ...
    return ReactionKnockoutOptimization(
        model=model, simulation_method=fba, objective_function=objective
    )
...
class TestReactionKnockoutOptimization:
    def test_initializer(self, model):
        ...
        rko = ReactionKnockoutOptimization(
            model=model, simulation_method=fba, objective_function=objective
        )
    ...
    def test_run_single_objective(self, reaction_ko_single_objective):
        ...
        results = reaction_ko_single_objective.run(
            max_evaluations=3000, pop_size=10, view=SequentialView(), seed=SEED
        )
        assert len(results.data_frame.targets) > 0
        assert len(results.data_frame.targets) == len(
            results.data_frame.targets.apply(tuple).unique()
        )
    ...
    def test_run_reaction_single_ko_objective_benchmark(
        self, benchmark, reaction_ko_single_objective
    ):
        benchmark(
            reaction_ko_single_objective.run,
    ...
    def test_run_multi_objective(self, model, reaction_ko_multi_objective):
        # TODO: make optlang deterministic so this results can be permanently stored.
        _, result_file = mkstemp(".pkl")
        results = reaction_ko_multi_objective.run(
            max_evaluations=3000, pop_size=10, view=SequentialView(), seed=SEED
        )
    ...
    def test_run_reaction_ko_multi_objective_benchmark(
        self, benchmark, reaction_ko_multi_objective
    ):
        benchmark(
            reaction_ko_multi_objective.run,
            max_evaluations=3000,
            pop_size=10,
            view=SequentialView(),
            seed=SEED,
...
def gene_ko_single_objective(model):
    ...
    return GeneKnockoutOptimization(
        model=model, simulation_method=fba, objective_function=objective
    )
...
class TestGeneKnockoutOptimization:
    ...
    def test_run_single_objective(self, model, gene_ko_single_objective):
        # TODO: make optlang deterministic so this results can be permanently stored.
        _, result_file = mkstemp(".pkl")
        results = gene_ko_single_objective.run(
            max_evaluations=3000, pop_size=10, view=SequentialView(), seed=SEED
        )
    ...
    def test_run_gene_ko_single_objective_benchmark(
        self, gene_ko_single_objective, benchmark
    ):
        benchmark(
            gene_ko_single_objective.run,
            max_evaluations=3000,
            pop_size=10,
            view=SequentialView(),
            seed=SEED,
    ...
    def test_run_multi_objective(self, model, gene_ko_multi_objective):
        ...
        results = gene_ko_multi_objective.run(
            max_evaluations=3000, pop_size=10, view=SequentialView(), seed=SEED
        )
    ...
    def test_run_gene_ko_multi_objective_benchmark(
        self, gene_ko_multi_objective, benchmark
    ):
        benchmark(
            gene_ko_multi_objective.run,
            max_evaluations=3000,
            pop_size=10,
            view=SequentialView(),
            seed=SEED,
...
class TestVariator:
    def test_set_n_point_crossover(self):
        ...
        args = {"crossover_rate": 1.0, "num_crossover_points": 1, "candidate_size": 10}
    ...
    def test_do_not_set_n_point_crossover(self):
        mom = OrderedSet([1, 3, 5, 9, 10])
        dad = OrderedSet([2, 3, 7, 8])
        args = {"crossover_rate": 0.0, "num_crossover_points": 1, "candidate_size": 10}
        children = set_n_point_crossover(Random(SEED), [mom, dad], args)
    ...
    def test_set_mutation(self):
        ...
        args = {"representation": representation, "mutation_rate": 1.0}
    ...
    def test_do_not_set_mutation(self):
        individual = OrderedSet([1, 3, 5, 9, 10])
        representation = list(range(10))
        args = {"representation": representation, "mutation_rate": 0.0}
        new_individuals = set_mutation(Random(SEED), [individual], args)
        assert len(new_individuals[0]) == len(individual)
        assert new_individuals[0] == individual
    ...
    def test_set_indel(self):
        ...
        representation = list(range(10))
        args = {"representation": representation, "indel_rate": 1.0}
    ...
    def test_do_not_set_indel(self):
        individual = [1, 3, 5, 9, 10]
        representation = list(range(10))
        args = {"representation": representation, "indel_rate": 0.0}
        new_individuals = set_indel(Random(SEED), [individual], args)
        assert len(new_individuals[0]) == len(individual)
        assert new_individuals[0] == individual
    ...
    def test_do_set_n_point_crossover(self):
        ...
        mom = OrderedSet(
            [representation.index(v) for v in ["A", "B", "E", "K", "L", "M"]]
        )
        dad = OrderedSet(
            [representation.index(v) for v in ["A", "C", "I", "J", "K", "L"]]
        )
        points = [4]
        children = _do_set_n_point_crossover(
            int_representation, mom, dad, points, Random(), len(mom)
        )
    ...
    def test_multiple_chromosome_set_indel(self):
        ...
        random = Random(SEED)
        new_individuals = multiple_chromosome_set_indel(
            random, [genome for _ in range(5)], args
        )
...
def simplify_knockout_solutions_for_succ(iaf1260):
    ...
    bpcy = biomass_product_coupled_min_yield(
        "Ec_biomass_iAF1260_core_59p81M", "EX_succ_lp_e_rp_", "EX_glc_lp_e_rp_"
    )
