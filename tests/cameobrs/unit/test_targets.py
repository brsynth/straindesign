...
class TestTargets:
    ...
    def test_gene_knockout_target(self, model):
        gene = "b4025"
        knockout_target = GeneKnockoutTarget(
            gene, accession_id=gene, accession_db="bigg"
        )
        knockout_target_gnomic = knockout_target.to_gnomic()
        assert (
            genotype_to_string(Genotype([knockout_target_gnomic]))
            == "-b4025#bigg:b4025"
        )
        with model:
            knockout_target.apply(model)
    ...
    def test_gnomic_integration_FluxModulationTarget(self, model):
        ...
        flux_modulation_target = FluxModulationTarget(
            "PGK", 0.5, 1, accession_id="PGK", accession_db="bigg"
        )
        flux_modulation_target_gnomic = flux_modulation_target.to_gnomic()
    ...
    def test_gnomic_integration_ReactionKnockinTarget(self, model):
        ...
        assert (
            genotype_to_string(Genotype([knockin_target_gnomic])) == "+reaction.atpzase"
        )

        reaction.add_metabolites({model.metabolites.atp_c: 1, atp_z: -1})
        knockin_target = ReactionKnockinTarget(
            "atpzase", reaction, accession_id="atpzase", accession_db="unicorn"
        )
        knockin_target_gnomic = knockin_target.to_gnomic()
        assert (
            genotype_to_string(Genotype([knockin_target_gnomic]))
            == "+reaction.atpzase#unicorn:atpzase"
        )
...
class TestEnsembleTargets:
    def test_compatible_targets(self):
        ...
        swap_target = ReactionCofactorSwapTarget(
            "b", [("nad_c", "nadh_c"), ("nadp_c", "nadph_c")]
        )
