...
from cameobrs.flux_analysis import remove_infeasible_cycles, structural
...
from cameobrs.flux_analysis.simulation import fba, lmoma, moma, pfba, room
from cameobrs.flux_analysis.structural import nullspace
from cameobrs.parallel import MultiprocessingView, SequentialView
from cameobrs.util import ProblemCache, current_solver_name, pick_one
...
def assert_data_frames_equal(obj, expected, delta=0.001, sort_by=None):
    df = obj.data_frame
    expected_names = [
        name for name in expected.columns.values if not re.match(r"^Unnamed.*", name)
    ]
...
class TestFluxVariabilityAnalysis:
    def test_flux_variability_parallel(self, core_model, ref_fva):
        ...
        pfba_fva = flux_variability_analysis(
            core_model, fraction_of_optimum=1, pfba_factor=1, view=mp_view
        ).data_frame
        mp_view.shutdown()
        assert_data_frames_equal(fva_solution, ref_fva)
    ...
    def test_flux_variability_parallel_remove_cycles(self, core_model, ref_fva):
        original_objective = core_model.objective
        fva_solution = flux_variability_analysis(
            core_model,
            fraction_of_optimum=0.999999419892,
        ...
        assert ref_fva["upper_bound"]["FRD7"] > 666.0
        assert round(abs(fva_solution["upper_bound"]["FRD7"] - 0.0), 7) == 0
        for key in fva_solution.data_frame.index:
            if ref_fva["lower_bound"][key] > -666:
                assert (
                    abs(fva_solution["lower_bound"][key] - ref_fva["lower_bound"][key])
                    < 0.0001
                )
            if ref_fva["upper_bound"][key] < 666:
                assert (
                    abs(fva_solution["upper_bound"][key] - ref_fva["upper_bound"][key])
                    < 0.0001
                )
        assert original_objective == core_model.objective

    def test_flux_variability_sequential(self, core_model, ref_fva):
        original_objective = core_model.objective
        fva_solution = flux_variability_analysis(
            core_model,
            fraction_of_optimum=0.999999419892,
        ...
        pfba_fva = flux_variability_analysis(
            core_model, fraction_of_optimum=1, pfba_factor=1
        ).data_frame
        assert_data_frames_equal(fva_solution, ref_fva)
        assert original_objective == core_model.objective
        assert sum(abs(pfba_fva.lower_bound)) - 518.422 < 0.001
    ...
    def test_flux_variability_sequential_remove_cycles(self, core_model, ref_fva):
        original_objective = core_model.objective
        fva_solution = flux_variability_analysis(
            core_model,
            fraction_of_optimum=0.999999419892,
        ...
        core_model.add_reactions([cycle_reac])
        cycle_reac.add_metabolites(
            {met: -c for met, c in core_model.reactions.PGI.metabolites.items()}
        )
        fva_solution = flux_variability_analysis(
            core_model, remove_cycles=False, reactions=["PGI"]
        )
        assert fva_solution.data_frame.loc["PGI", "upper_bound"] == 1000
        fva_solution = flux_variability_analysis(
            core_model, remove_cycles=True, reactions=["PGI"]
        )
...
class TestPhenotypicPhasePlane:
    ...
    def test_one_variable_parallel(self, core_model, ref_ecore):
        ppp = phenotypic_phase_plane(
            core_model, ["EX_o2_LPAREN_e_RPAREN_"], view=MultiprocessingView()
        )
        assert_data_frames_equal(ppp, ref_ecore, sort_by=["EX_o2_LPAREN_e_RPAREN_"])
        ppp = phenotypic_phase_plane(
            core_model, "EX_o2_LPAREN_e_RPAREN_", view=MultiprocessingView()
        )
        assert_data_frames_equal(ppp, ref_ecore, sort_by=["EX_o2_LPAREN_e_RPAREN_"])

    def test_one_variable_sequential(self, core_model, ref_ecore):
        ppp = phenotypic_phase_plane(
            core_model, ["EX_o2_LPAREN_e_RPAREN_"], view=SequentialView()
        )
        assert_data_frames_equal(ppp, ref_ecore, sort_by=["EX_o2_LPAREN_e_RPAREN_"])
        ppp = phenotypic_phase_plane(
            core_model, "EX_o2_LPAREN_e_RPAREN_", view=SequentialView()
    ...
    def test_one_variable_sequential_yield(self, core_model, ref_ecore):
        ppp = phenotypic_phase_plane(
            core_model, ["EX_o2_LPAREN_e_RPAREN_"], view=SequentialView()
        )
        assert_data_frames_equal(ppp, ref_ecore, sort_by=["EX_o2_LPAREN_e_RPAREN_"])
    ...
    @pytest.mark.skipif(CI, reason="Running in Travis")
    def test_two_variables_parallel(self, core_model, ref_ecore_glc):
        ppp2d = phenotypic_phase_plane(
            core_model,
            ["EX_o2_LPAREN_e_RPAREN_", "EX_glc_LPAREN_e_RPAREN_"],
            view=MultiprocessingView(),
        )
    ...
    def test_two_variables_sequential(self, core_model, ref_ecore_glc):
        ppp2d = phenotypic_phase_plane(
            core_model,
            ["EX_o2_LPAREN_e_RPAREN_", "EX_glc_LPAREN_e_RPAREN_"],
            view=SequentialView(),
    ...
    def test_one_variable_sequential_metabolite(self, core_model, ref_ecore_ac):
        ppp = phenotypic_phase_plane(
            core_model,
            ["EX_o2_LPAREN_e_RPAREN_"],
            core_model.metabolites.ac_c,
            view=SequentialView(),
        )
        assert_data_frames_equal(ppp, ref_ecore_ac, sort_by=["EX_o2_LPAREN_e_RPAREN_"])
...
class TestSimulationMethods:
    ...
    def test_fba_with_reaction_filter(self, core_model):
        original_objective = core_model.objective
        solution = fba(
            core_model, reactions=["EX_o2_LPAREN_e_RPAREN_", "EX_glc_LPAREN_e_RPAREN_"]
        )
        assert abs(solution.objective_value - 0.873921) < 0.000001
    ...
    def test_pfba(self, core_model):
        ...
        ) < 1e-6, "FBA sum is suppose to be lower than PFBA (was %f)" % (
            pfba_flux_sum - fba_flux_sum
        )
        assert core_model.objective.expression == original_objective.expression
    ...
    def test_lmoma(self, core_model):
        ...
        distance = sum(
            (abs(solution[v] - pfba_solution[v]) for v in pfba_solution.keys())
        )
        assert abs(0 - distance) < 1e-6, (
            "lmoma distance without knockouts must be 0 (was %f)" % distance
        )
        assert core_model.objective.expression == original_objective.expression
    ...
    def test_lmoma_change_ref(self, core_model):
        ...
        assert not any(
            c.name.startswith("lmoma_const_") for c in core_model.solver.constraints
        )
    ...
    def test_moma(self, core_model):
        ...
        assert abs(0 - distance) < 1e-6, (
            "moma distance without knockouts must be 0 (was %f)" % distance
        )
        assert core_model.objective.expression == original_objective.expression
        assert not any(
            v.name.startswith("moma_aux_") for v in core_model.solver.variables
        )
        assert not any(
            c.name.startswith("moma_const_") for c in core_model.solver.constraints
        )
    ...
    def test_room(self, core_model):
        ...
        assert abs(0 - solution.objective_value) < 1e-6, (
            "room objective without knockouts must be 0 (was %f)"
        ...
        for r in core_model.reactions:
            if r not in essential_reactions:
                with core_model:
                    ...
                    assert any(
                        v.name.startswith("moma_aux_")
                        for v in core_model.solver.variables
                    )
                    assert any(
                        c.name.startswith("moma_const_")
                        for c in core_model.solver.constraints
                    )
        cache.reset()
        assert core_model.objective.expression == original_objective.expression
        assert not any(
        ...
        for r in core_model.reactions:
            if r not in essential_reactions:
                with core_model:
                    ...
                    assert any(
                        v.name.startswith("u_") for v in core_model.solver.variables
                    )
                    assert any(
                        c.name.startswith("lmoma_const_")
                        for c in core_model.solver.constraints
                    )
        cache.reset()
        assert core_model.objective.expression == original_objective.expression
    ...
    def test_room_with_cache(self, core_model):
            ...
            if r not in essential_reactions:
                with core_model:
                    ...
                    try:
                        ...
                        assert any(
                            v.name.startswith("y_") for v in core_model.solver.variables
                        )
                        assert any(
                            c.name.startswith("room_const_")
                            for c in core_model.solver.constraints
                        )
        ...
        assert not any(v.name.startswith("y_") for v in core_model.solver.variables)
        assert not any(
            c.name.startswith("room_const_") for c in core_model.solver.constraints
        )
...
class TestStructural:
    ...
    def test_find_coupled_reactions(self, core_model):
        ...
        for coupled_set in couples:
            ...
            assert (
                round(abs(fluxes[coupled_set[0].id] - fluxes[coupled_set[1].id]), 7)
                == 0
            )

        couples, blocked = structural.find_coupled_reactions(
            core_model, return_dead_ends=True
        )
    ...
    def test_dead_end_metabolites_are_in_dead_end_reactions(self, core_model):
        ...
        dead_end_metabolites = {
            m for m in core_model.metabolites if len(m.reactions) == 1
        }
        ...
        for essential_reaction in essential_reactions:
            for group in coupled_reactions:
                ...
                if essential_reaction in group:
                    assert all(
                        group_reaction in essential_reactions
                        for group_reaction in group
                    )
        ...
        for group in coupled_reactions:
            ...
            if representative not in essential_reactions:
                with core_model:
                    ...
                    assert (
                        representative.forward_variable
                        not in core_model.solver.variables
                    )
                    assert (
                        representative.reverse_variable
                        not in core_model.solver.variables
                    )
                    assert representative not in core_model.reactions
                    ...
                    blocked_reactions = find_blocked_reactions(core_model)
                    assert all(
                        r in blocked_reactions for r in group if r != representative
                    )
