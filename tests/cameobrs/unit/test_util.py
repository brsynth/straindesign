...
from cameobrs.network_analysis.util import distance_based_on_molecular_formula
from cameobrs.util import (
    ProblemCache,
    RandomGenerator,
    Singleton,
    TimeMachine,
...
class TestTimeMachine:
    def test_one_change_list(self):
        tm = TimeMachine()
        li = [1, 2, 3, 4]
        tm(do=partial(li.append, 5), undo=li.pop)
        assert li == [1, 2, 3, 4, 5]
        tm.reset()
        assert li == [1, 2, 3, 4]
    ...
    def test_str_handles_different_types_of_stored_operations(self):
        ...
        assert tm.__str__().split("\n")[2:-1] == [
            "undo: " + str(str) + " (1,) {}",
            "redo: normal_function",
        ]

    def test_with_statement(self):
        li = [1, 2, 3, 4]
        with TimeMachine() as tm:
            tm(do=partial(li.append, 33), undo=partial(li.pop))
            tm(do=partial(li.append, 66), undo=partial(li.pop))
            tm(do=partial(li.append, 99), undo=partial(li.pop))
        assert li == [1, 2, 3, 4]
...
def some_method_that_adds_stuff(model, cache):
    ...
    for i in range(9):
        ...
        cache.add_constraint(
            "c_%i" % (i + 1), create_constraint, update_constraint, [v1, v2], -20, 100
        )
...
class TestProblemCache:
    ...
    def test_add_constraint(self, core_model):
        ...
        for i in range(10):
            ...
            cache.add_constraint(
                "c%i" % i, add_constraint, update_constraint, cache.variables["%i" % i]
            )
...
class TestUtils:
    ...
    def test_distance_based_on_molecular_formula(self):  # from network_analysis.util
        ...
        assert (
            distance_based_on_molecular_formula(met1, met2, normalize=True) == 1.0 / 7
        )

        assert distance_based_on_molecular_formula(met2, met3, normalize=False) == 20
        assert (
            distance_based_on_molecular_formula(met2, met3, normalize=True) == 20.0 / 28
        )

        assert distance_based_on_molecular_formula(met1, met3, normalize=False) == 21
        assert (
            distance_based_on_molecular_formula(met1, met3, normalize=True) == 21.0 / 27
        )
