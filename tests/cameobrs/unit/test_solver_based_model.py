...
from cobra.util import SolverNotFound, fix_objective_as_constraint

from cameobrs import load_model
from cameobrs.config import solvers
from cameobrs.core.utils import get_reaction_for, load_medium, medium
from cameobrs.flux_analysis.analysis import find_essential_metabolites
from cameobrs.flux_analysis.structural import create_stoichiometric_array

CI = bool(os.getenv("CI", False))
TESTDIR = os.path.dirname(__file__)
# REFERENCE_FVA_SOLUTION_ECOLI_CORE = pandas.read_csv(
#    os.path.join(TESTDIR, "data/REFERENCE_flux_ranges_EcoliCore.csv"), index_col=0
# )
ESSENTIAL_GENES = [
    "b2779",
    "b1779",
...
class TestReaction:
    ...
    def test_gene_reaction_rule_setter_reaction_already_added_to_model(
        self, core_model
    ):
        rxn = Reaction("rxn")
        rxn.add_metabolites({Metabolite("A"): -1, Metabolite("B"): 1})
        core_model.add_reactions([rxn])
    ...
    def test_add_metabolite(self, solved_model):
        ...
        assert (
            constraints[test_met.id].expression.as_coefficients_dict()[
    ...
    def test_make_lhs_irreversible_reversible(self, core_model):
        rxn = Reaction("test")
        rxn.add_metabolites(
            {core_model.metabolites[0]: -1.0, core_model.metabolites[1]: 1.0}
        )
    ...
    def test_add_metabolites_combine_true(self, core_model):
        ...
        for reaction in core_model.reactions:
            ...
            assert core_model.solver.constraints["test"].expression.has(
            ...
            assert core_model.solver.constraints[
                already_included_metabolite.id
            ].expression.has(new_coefficient * reaction.forward_variable)
            assert core_model.solver.constraints[
                already_included_metabolite.id
            ].expression.has(-1 * new_coefficient * reaction.reverse_variable)
    ...
    def test_add_metabolites_combine_false(self, core_model):
        ...
        for reaction in core_model.reactions:
            ...
            assert core_model.solver.constraints[
                already_included_metabolite.id
            ].expression.has(10 * reaction.forward_variable.name)
            assert core_model.solver.constraints[
                already_included_metabolite.id
            ].expression.has(-10 * reaction.reverse_variable.name)
...
class TestModel:
    ...
    def test_objective_coefficient_reflects_changed_objective(self, core_model):
        biomass_r = core_model.reactions.get_by_id(
            "Biomass_Ecoli_core_N_LPAREN_w_FSLASH_GAM_RPAREN__Nmet2"
        )
    ...
    def test_add_reactions(self, core_model):
        ...
        assert isinstance(
            core_model.reactions[-2].reverse_variable,
            core_model.solver.interface.Variable,
        )
        coefficients_dict = (
            core_model.solver.objective.expression.as_coefficients_dict()
        )
        biomass_r = core_model.reactions.get_by_id(
            "Biomass_Ecoli_core_N_LPAREN_w_FSLASH_GAM_RPAREN__Nmet2"
        )
        ...
        assert coefficients_dict[core_model.reactions.r2.forward_variable] == 3.0
        assert coefficients_dict[core_model.reactions.r2.reverse_variable] == -3.0

    def test_remove_reactions_1(self, core_model):
        core_model.remove_reactions(
            [core_model.reactions.PGI, core_model.reactions.PGK]
        )
    ...
    def test_remove_reactions_2(self, core_model):
        ...
        assert all(
            [
                core_model.reactions.get_by_id(reaction.id) == reaction
    ...
    def test_objective(self, core_model):
        ...
        assert {
            var.name: coef
            for var, coef in obj.expression.as_coefficients_dict().items()
        } == {
    ...
    def test_change_objective(self, core_model):
        expression = (
            1.0 * core_model.solver.variables["ENO"]
            + 1.0 * core_model.solver.variables["PFK"]
        )
    ...
    def test_invalid_objective_raises(self, core_model):
        ...
        with pytest.raises(TypeError):
            setattr(core_model, "objective", 3.0)
    ...
    def test_essential_genes(self, core_model):
        ...
        with pytest.raises(OptimizationError):
            core_model.reactions.Biomass_Ecoli_core_N_LPAREN_w_FSLASH_GAM_RPAREN__Nmet2.bounds = (
                999999,
                999999,
            )
    ...
    def test_essential_reactions(self, core_model):
        observed_essential_reactions = [
            r.id for r in find_essential_reactions(core_model)
        ]
    ...
    def test_essential_metabolites_steady_state(self, core_model):
        essential_metabolites_balanced = [
            m.id
            for m in find_essential_metabolites(core_model, force_steady_state=True)
        ]
    ...
    @pytest.mark.xfail(
        reason="needs some refactoring, uses missing bounds, not allowed by cplex"
    )
    def test_essential_metabolites(self, core_model):
        essential_metabolites_unbalanced = [
            m.id
            for m in find_essential_metabolites(core_model, force_steady_state=False)
        ]
    ...
    def test_fix_objective_as_constraint(self, core_model):
        ...
        with core_model:
            ...
            assert (
                core_model.solver.constraints[-1].expression
                - core_model.objective.expression
                == 0
            )
    ...
    def test_solver_change_preserves_non_metabolic_constraints(self, core_model):
        with core_model:
            constraint = core_model.problem.Constraint(
                core_model.reactions.PGK.flux_expression
