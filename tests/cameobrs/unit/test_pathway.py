...
from cameobrs.core.pathway import Equation, Pathway
...
def but_met():
    mets = {}
    mets["b2coa_c"] = Metabolite(id="b2coa_c", name="Crotonoyl-CoA", compartment="c")
    mets["nadh_c"] = Metabolite(
        id="nadh_c", name="Nicotinamide adenine dinucleotide - reduced", compartment="c"
    )
    mets["h_c"] = Metabolite(id="h_c", name="H+", compartment="c")
    mets["btcoa_c"] = Metabolite(
        id="btcoa_c", name="Gamma-butyrobetainyl-CoA", compartment="c"
    )
    mets["nad_c"] = Metabolite(id="nad_c", name="Deamino-NAD+", compartment="c")
    return mets
...
class TestEquation:
    ...
    def test_tokenize(self, but_df, but_met):
        ...
        ths = []
        ths.append(
            Equation.Token(kind="COEFF", value=1, group="reactant", target=False)
        )
        ths.append(
            Equation.Token(
                kind="MET", value=but_met["b2coa_c"], group="reactant", target=False
            )
        )
        ths.append(
            Equation.Token(kind="COEFF", value=1, group="reactant", target=False)
        ...
        ths.append(
            Equation.Token(
                kind="MET", value=but_met["nadh_c"], group="reactant", target=False
            )
        )
        ths.append(
            Equation.Token(kind="COEFF", value=1, group="reactant", target=False)
        ...
        ths.append(
            Equation.Token(
                kind="MET", value=but_met["h_c"], group="reactant", target=False
            )
        )
        ths.append(
            Equation.Token(kind="ARROW", value="=> ", group="product", target=False)
        )
        ths.append(Equation.Token(kind="COEFF", value=1, group="product", target=False))
        ths.append(
            Equation.Token(
                kind="MET", value=but_met["btcoa_c"], group="product", target=False
            )
        )
        ths.append(Equation.Token(kind="COEFF", value=1, group="product", target=False))
        ths.append(
            Equation.Token(
                kind="MET", value=but_met["nad_c"], group="product", target=False
            )
        )
